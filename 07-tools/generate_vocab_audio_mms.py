...
def minimal_declick(audio: np.ndarray) -> np.ndarray:
    """Apply gentle denoising: high-pass + subtle smoothing."""
    # Very gentle high-pass to remove DC offset and clicks (50 Hz)
    sos = scipy.signal.butter(2, 50, 'hp', fs=16000, output='sos')
    audio = scipy.signal.sosfilt(sos, audio)
    
    # Apply median filter for artifact reduction (3-tap, very gentle smoothing)
